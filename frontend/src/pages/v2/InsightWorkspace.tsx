...
export default function InsightWorkspace() {
  ...
  const kind=query.get('view')==='monthly'?'monthly':'weekly';const period=kind==='monthly'?'月':'周';
  ...
  useEffect(() => {
    let active = true;
    setError(''); setReports([]);
    void insightApi.list(group, history,kind).then(r => { if (active) setReports(r.items); }).catch(e => { if (active) setError(String(e)); });
    return () => { active = false; };
  }, [group, history, refresh,kind]);
  useEffect(() => {
    let active = true;
    setReport(undefined); setSources(undefined);
    if (reportId > 0) void insightApi.detail(reportId).then(r => { if (active) {if(r.kind===kind)setReport(r);else setError('报告类型与当前标签不一致，请切换对应周期。');} }).catch(e => { if (active) setError(String(e)); });
    return () => { active = false; };
  }, [reportId, refresh]);
  async function build() {
    setBusy(true); setError('');
    try { const task = await insightApi.build(Number(group), day,kind); setNotice(`计算任务 #${task.job_id} 已提交，完成后刷新作品列表。`); }
  ...
  return <div className="knowledge-page insight-workspace">
    <section><h2>{kind==='monthly'?'Monthly Insight · 月度回顾':'Weekly Insight · 周度变化'}</h2><p>{kind==='monthly'?'从原消息与记忆进展汇总整月节奏和长期变化，不拼接周报正文。':'按消息发生日期计算自然周，话题与事件来自有来源的长期记忆。'}</p>
      ...
      <button disabled={busy || !group || !day} onClick={() => void build()}>计算{period}度洞察</button>
    ...
    <div className="insight-columns"><section><h2>{period}报列表</h2>{!reports.length && <p>尚无周期洞察。数据底座就绪后可以计算；旧日报仍可在日报工作区查看。</p>}
      {reports.map(r => <article key={r.id}><p>{groups.find(g => g.id === r.group_id)?.display_name || `群 ${r.group_id}`} · {new Date(r.period_start).toLocaleDateString('zh-CN')} 起</p>
        <button disabled={busy} onClick={() => updateWorkspaceQuery({ insight: String(r.id), message: null })}>查看{period}报 #{r.id} · v{r.revision}</button><p>{r.status === 'READY' ? '统计完整' : '统计尚未完整'}</p></article>)}
    </section>
    {report && <section aria-label="周期洞察详情"><h2>本{period}概况 · v{report.revision}</h2>
      ...
      {kind==='monthly'&&<><h3>整月节奏</h3><p>{report.metrics.month_days} 天 · 已知日均 {report.metrics.known_daily_average} 条消息；数据不完整时为已知下界。</p><h3>自然周在本月的活跃趋势</h3>{report.metrics.weekly_trends?.map(w=><p key={w.week_start}>{w.from}—{w.through}（{w.days} 天）：{w.known_messages} 条{w.complete?'':'（已知，覆盖不完整）'}</p>)}<h3>月度关键词</h3><p>{report.metrics.keyword_basis}</p><p>{report.metrics.keywords?.map(k=>`${k.word}（${k.message_count} 条消息）`).join(' · ')||'暂无稳定词频'}</p></>}<h3>全员排行与变化</h3><p>口径：{report.metrics.count_policy === 'text_primary' ? '文字优先' : '全部可计数消息'}；同分按现有稳定姓名规则排序。</p>
      ...
      <p>{period}冠军：{report.metrics.champion?.name || '覆盖或身份不足，暂不确认'}</p>
      ...
      {report.sections?.map(s=><article key={s.key}><h4>{s.title}</h4>{s.note&&<p>{s.note}</p>}{s.summary&&<p>{s.summary}</p>}{s.review_status==='review'&&<p>该记忆仍待核对</p>}
        {s.items?.slice(0,15).map(t=><p key={t.memory_id}>{t.title} · {t.discussion_days} 个讨论日 · {t.participants} 位参与者 · {t.evidence_messages} 条证据 · {t.lifecycle==='newly_observed'?'首次观察到':t.lifecycle==='continuing'?'持续讨论':t.lifecycle==='not_observed'?'本期未再次观察到':'再次活跃'}</p>)}
