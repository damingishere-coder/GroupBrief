...
【空间不足时的降级顺序】
严格依次减少：装饰 → 底部总结 → 副标题 → 次要气泡。不得删除主标题、统计日期、两项统计数据、任何入选话题、完整真实姓名、事实短句或主气泡。
...
【重新生图不变量】
重新生图时只允许改变所选美术家族和当天已解析的画材、配色、纹理、光影细节；聊天事实、统计日期、数字、人物、逐字气泡、话题覆盖和既定漫画分镜不得改变。
...
15. 空间不足时只能按既定降级顺序缩减，日期、两项统计、全部话题、真实姓名、事实短句和主气泡是不可删除项。
"""
