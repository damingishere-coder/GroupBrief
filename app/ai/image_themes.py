...
IMAGE_THEME_MODE_DEFINITIONS: dict[str, ImageThemeDefinition] = {
    RANDOM_PRESET_THEME: ImageThemeDefinition(RANDOM_PRESET_THEME, "每日随机", "每群每天独立生成一套可复现风格", "按日期与群聊稳定生成兼容的画材、配色、版式、纹理和光影组合。"),
...
_STYLE_FAMILIES: tuple[dict[str, Any], ...] = (
    {"label": "丝网印刷社论漫画", "media": ("颗粒丝网印刷", "双色孔版印刷"), "palette": ("群青、奶油白与珊瑚红", "墨绿、米白与亮橙"), "layout": ("粗标题与错位分栏", "海报式大标题与模块卡片"), "texture": ("纸张颗粒和套色轻微错位", "半调网点与撕纸边缘"), "light": ("平面高对比光影", "清晰块面明暗")},
    {"label": "纸艺立体信息图", "media": ("分层剪纸", "立体纸雕"), "palette": ("天蓝、暖黄与番茄红", "鼠尾草绿、杏色与靛蓝"), "layout": ("纵向舞台式分区", "层叠卡片式信息架构"), "texture": ("纤维纸纹与圆润切边", "折纸阴影与手工拼贴边缘"), "light": ("柔和棚拍侧光", "浅景深纸艺投影")},
    {"label": "水彩旅行手账", "media": ("透明水彩线稿", "水彩与彩铅混合"), "palette": ("湖蓝、浅赭与豆沙红", "薄荷绿、柠檬黄与灰紫"), "layout": ("手账页签与路线式阅读", "留白充足的散点分栏"), "texture": ("水渍晕染和纸胶带", "彩铅笔触与手写箭头"), "light": ("通透自然光", "柔和晨光氛围")},
    {"label": "复古未来主义", "media": ("复古科幻杂志插画", "几何矢量科幻海报"), "palette": ("深靛蓝、铜橙与象牙白", "紫黑、青绿与暖金"), "layout": ("控制台仪表分区", "放射透视与悬浮信息窗"), "texture": ("老印刷网点与金属刻度", "扫描线和细密星尘"), "light": ("边缘霓虹与局部辉光", "戏剧化逆光")},
    {"label": "黏土定格剧场", "media": ("手工黏土定格动画", "软陶微缩场景"), "palette": ("奶油黄、天空蓝与草莓红", "陶土橙、青绿与乳白"), "layout": ("微缩舞台分镜", "玩具展示柜式分区"), "texture": ("可见手捏纹理与圆角道具", "软陶颗粒和纸板布景"), "light": ("温暖摄影棚柔光", "微缩场景侧逆光")},
    {"label": "木刻新闻画", "media": ("现代木刻版画", "粗线条凸版印刷"), "palette": ("黑、米白与朱红", "藏青、牛皮纸色与橘黄"), "layout": ("报刊网格与醒目栏题", "纵向连环画分格"), "texture": ("刀刻线纹和粗粝纸面", "油墨不均与印章点缀"), "light": ("高反差明暗", "版画式硬边阴影")},
    {"label": "玻璃拟态科技刊", "media": ("半透明玻璃拟态", "清洁三维信息面板"), "palette": ("冰蓝、白色与荧光青", "深灰蓝、薰衣草紫与亮粉"), "layout": ("纵向数据面板与浮层卡片", "中心主视觉加环绕模块"), "texture": ("磨砂玻璃和细线网格", "透明折射与柔和渐变"), "light": ("冷色体积光", "柔亮边缘光")},
    {"label": "儿童科普绘本", "media": ("蜡笔与彩铅绘本", "不透明水粉童书插画"), "palette": ("明黄、草绿与天空蓝", "珊瑚红、奶油白与海军蓝"), "layout": ("故事路径式分区", "大图小卡片的绘本跨页"), "texture": ("蜡笔颗粒和手绘图标", "水粉笔触与圆润贴纸"), "light": ("明快均匀光线", "温暖午后光")},
    {"label": "建筑蓝图漫画", "media": ("工程蓝图线稿", "等距轴测技术插画"), "palette": ("蓝底白线与安全橙", "石墨灰、亮蓝与荧光黄"), "layout": ("标注线连接的信息节点", "等距场景与参数卡片"), "texture": ("网格纸、尺寸线和编号章", "铅笔辅助线与半透明图层"), "light": ("理性均匀照明", "细微高光强化结构")},
    {"label": "织物刺绣拼布", "media": ("刺绣与布艺拼贴", "羊毛毡立体插画"), "palette": ("靛蓝、米白与砖红", "橄榄绿、芥末黄与莓果紫"), "layout": ("拼布网格与缝线路径", "布章式标题与分区"), "texture": ("织物纹理、针脚和毛毡边缘", "绒线轮廓与布贴层次"), "light": ("柔和室内散射光", "温暖侧光突出纤维")},
)
...
def _daily_style(seed_text: str, previous_signature: str = "") -> ResolvedImageTheme:
    for attempt in range(20):
        ...
        layout = picker.choice(family["layout"])
        texture = picker.choice(family["texture"])
        light = picker.choice(family["light"])
        prompt = (
            f"统一采用{media}形式；配色为{palette}；版式使用{layout}；"
            f"加入{texture}；光影为{light}。保持中文大字清晰、信息层级明确，"
            "所有风格元素只作为视觉表现，不得新增或改变聊天事实。"
        )
...
def resolve_image_theme(
    ...
    *,
    ...
    if key == RANDOM_PRESET_THEME:
        if persisted_meta and persisted_meta.get("requested_theme") == RANDOM_PRESET_THEME and persisted_meta.get("theme_prompt"):
            return ResolvedImageTheme(
                RANDOM_PRESET_THEME,
    ...
    if key == CUSTOM_THEME:
        prompt = f"自定义大主题「{custom_text}」：严格使用该指定风格并完全替代随机风格；只能影响配色、装饰、服装、构图和画风，不能新增或改变聊天事实。"
